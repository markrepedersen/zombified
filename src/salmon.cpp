...
void Salmon::update(float ms)
{
	...
	if (m_is_alive)
	{
		...
		switch(key_pressed) {
			...
			break;
			case 1: if (m_position.y > viewPort[3]) {
				move({ 0, -step});
				press_key(0);
			} else {
				move({ 0, step});
			...
			break;
			case 3: if (m_position.x > viewPort[2]) {
				move({-step, 0});
				press_key(2);
			} else {
				move({step, 0});
...
void Salmon::draw(const mat3& projection)
{
	...
	float deadColor[] = {1.0, 0.0, 0.0 };

	if (!m_is_alive) {
		glUniform3fv(color_uloc, 1, deadColor);
	} else {
